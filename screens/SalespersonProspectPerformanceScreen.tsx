...
const LeadHistoryTimeline: React.FC<{ lead: ProspectAILead, allSalespeople: TeamMember[], pipeline: PipelineStage[], onImageClick: (url: string) => void }> = ({ lead, allSalespeople, pipeline, onImageClick }) => {
    const events = useMemo(() => generateLeadEvents(lead, allSalespeople, pipeline), [lead, allSalespeople, pipeline]);
    ...
    return (
        <div className="animate-fade-in">
                ...
                <Card className="p-4">
                     ...
                     <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                        {metrics.allFeedbacks.slice(0, 10).map((fb, index) => {
                            const events = generateLeadEvents(fb.lead, allSalespeople, companyPipeline);
                                    ...
                                    <div className="flex justify-between items-start gap-3">
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs text-dark-secondary">
                                                Lead: <span className="font-semibold text-dark-text">{fb.leadName}</span>
            ...
            <Modal isOpen={!!historyModalLead} onClose={() => setHistoryModalLead(null)}>
                {historyModalLead && <LeadHistoryTimeline lead={historyModalLead} allSalespeople={allSalespeople} pipeline={companyPipeline} onImageClick={setExpandedImageUrl}/>}
            </Modal>
